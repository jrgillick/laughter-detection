...
main_annotations_file = annotations_folder + \
    '/raw_annotations/Laughter_Annotations_2.csv'
...
for i in range(len(df.Start)):
    try:
        ...
        rowz.append(i)
        df.at[i, 'Start'] = 0.0
        # Not actually 0. Need to set this to length of audio file later.
        df.at[i, 'End'] = 0.0
    ...
    for i, line in enumerate(lines):
        ...
        for annotation_number, annotation in enumerate(row):
            ...
            if annotation_number == 0:
                ...
                h[f'Start.{annotation_number}'] = start_str
                h[f'End.{annotation_number}'] = end_str
...
df = df[['FileID', 'Start', 'End', 'Start.1', 'End.1',
         'Start.2', 'End.2', 'Start.3', 'End.3', 'Start.4', 'End.4']]

test_laughter_files = open(
    '../data/audioset/splits/test_laughter_files.txt').read().split('\n')
test_laughter_ids = open(
    '../data/audioset/splits/test_laughter_ids.txt').read().split('\n')
...
keys = [k for k in list(df.columns) if k not in [
    'FileID', 'audio_path', 'audio_length']]
boundary_errors = []
for i in range(len(df)):
    for key in keys:
        if float(df.at[i, key]) > float(df.at[i, 'audio_length']):
            boundary_errors.append(
                float(df.at[i, key]) - float(df.at[i, 'audio_length']))
            df.at[i, key] = df.at[i, 'audio_length']

print(
    f"Boundary Errors: {len(boundary_errors)} | Mean Error: {np.mean(boundary_errors)} | Max Error: {np.max(boundary_errors)}")
...
audioset_distractor_df.to_csv(
    '../data/audioset/annotations/clean_distractor_annotations.csv', index=None)
