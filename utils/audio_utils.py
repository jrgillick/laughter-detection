    ...
    # If there are no labels, then expect the batch of labels as [None, None...]
        ...
        if label_pad_value is not None:
            ...
            labels = pad_sequences(
                labels, label_pad_value, max_len=max_label_len)

        # Convert vocab to integers after padding
        if output_vocab is not None:
            labels = [text_utils.sequence_to_indices(
                l, output_vocab) for l in labels]

        if one_hot_labels:
            labels = [text_utils.np_onehot(
                l, depth=len(output_vocab)) for l in labels]
