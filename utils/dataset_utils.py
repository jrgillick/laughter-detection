...
def get_laughter_rows_from_file(f, include_words=False):
    if include_words:
        ...
        # doesn't allow laughter with words together
        return [l for l in get_text_from_file(f) if '[laughter]' in l]
...
def get_transcriptions_files(folder, a_or_b):
    ...
    for f in subfolders:
        fs = [
            f + "/" + fname for fname in os.listdir(f) if 'a-word.text' in fname and a_or_b in fname]
        files += fs
    files.sort()
    return files
...
def get_audio_files_from_transcription_files(transcription_files, all_audio_files):
    ...
    transcription_files = [
        t for t in transcription_files if t not in transcription_files_to_remove]
...
def get_laughter_regions_and_speech_regions(text_A, text_B, audio_file, random_seed=None, include_words=False):
    ...
    laughter_regions_A = get_laughter_regions_from_file(
        text_A, include_words=include_words)
    laughter_regions_B = get_laughter_regions_from_file(
        text_B, include_words=include_words)
    laughter_regions = combine_overlapping_regions(
        laughter_regions_A, laughter_regions_B)
    speech_regions = []
    ...
    for laughter_region in laughter_regions:
        ...
        speech_regions.append(get_random_speech_region_from_files(
            [text_A, text_B], audio_length, region_length, random_seed=random_seed))
    laughter_regions = [l for l in laughter_regions if l is not None]
    speech_regions = [s for s in speech_regions if s is not None]
    laughter_regions = [audio_utils.start_end_to_offset_duration(
        s, e) for s, e in laughter_regions]
    speech_regions = [audio_utils.start_end_to_offset_duration(
        s, e) for s, e in speech_regions]
...
def sample_switchboard_features_and_labels(audio_file, t_file_a, t_file_b, feature_fn, random_seed=None, include_words=False, **kwargs):
    ...
    speech_feats = audio_utils.featurize_audio_segments(
        speech_regions, feature_fn=feature_fn, f=audio_file, sr=8000)
    X = laughter_feats+speech_feats
    y = list(np.ones(len(laughter_regions))) + \
        list(np.zeros(len(speech_regions)))
    return list(zip(X, y))
