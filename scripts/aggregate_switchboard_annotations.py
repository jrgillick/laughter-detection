...
audioset_annotations_df = pd.read_csv(
    '../data/audioset/annotations/clean_laughter_annotations.csv')
...
class AllocationModel(nn.Module):
    def __init__(self, total_time, available_time_per_clip):
        ...
        self.allocated_times = nn.Parameter(torch.zeros(
            len(available_time_per_clip)), requires_grad=True)
    ...
    def forward(self):
        ...
        self.allocated_times.data = torch.min(
            self.allocated_times, self.available_time_per_clip)
        return loss, self.allocated_times


def distribute_time(total_time, available_time_per_clip):
    am = AllocationModel(torch.tensor(total_time),
                         torch.tensor(available_time_per_clip))
...
def get_10_second_clips(regions_list, audio_file_path, full_audio_file_length,
    ...
    for i, clip in enumerate(all_clips):
        ...
        if time_to_add_per_side < clip['beginning_space'] and time_to_add_per_side < clip['end_space']:
            clip['window'] = [start-time_to_add_per_side,
    ...
    for i, clip in enumerate(all_clips):
        ...
        # If longer than 10 secs, don't shorten it, just leave it
        time_to_add = np.maximum(10 - length, 0)
...
def make_switchboard_distractor_dataframe(t_files_A, t_files_B, a_files, total_distractor_clips=None):
    ...
    for i in tqdm(range(len(t_files_A))):
        ...
        laughter_regions = [{'start': r[0], 'end':r[0]+r[1]}
                            for r in laughter_regions]
...
swb_val_distractor_df.to_csv(
    '../data/switchboard/annotations/clean_switchboard_val_distractor_annotations.csv', index=None)
...
swb_test_distractor_df.to_csv(
    '../data/switchboard/annotations/clean_switchboard_test_distractor_annotations.csv', index=None)
...
swb_train_df.to_csv(
    '../data/switchboard/annotations/clean_switchboard_train_laughter_annotations.csv', index=None)
...
swb_val_df.to_csv(
    '../data/switchboard/annotations/clean_switchboard_val_laughter_annotations.csv', index=None)
...
swb_test_df.to_csv(
    '../data/switchboard/annotations/clean_switchboard_test_laughter_annotations.csv', index=None)
