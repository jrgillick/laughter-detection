...
class AudioDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, filepaths, feature_fn=None, label_paths=None,
        ...
        if not feature_and_label_fn and (bool(label_paths) ^ bool(label_fn)):
            raise Exception(
                "Can't use only one of `label_paths` and `label_fn`")

        self.include_labels = bool(label_paths) or bool(
            labels) or self.does_subsample
...
class SwitchBoardLaughterDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.subsample:
            audio_file_length = librosa.core.samples_to_time(
                len(audio_file), sr=self.sr)
