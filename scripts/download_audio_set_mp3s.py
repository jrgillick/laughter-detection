...
def get_laughter_infolist(csv_file, mode):
    ...
    np.random.seed(0)
    non_laughter_lines = list(np.random.choice(
        all_non_laughter_lines, n_examples, replace=False))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--num_processes',
                        help='number of parallel processes', required=True)
    # Set mode to download positive examples, negative examples, or both
    parser.add_argument(
        '-m', '--mode', help='{positive, negative, both}', required=True)
    ...
    if num_processes > 1:
        Parallel(n_jobs=num_processes)(delayed(download_laughter_audio_file)(
            laughter_info, output_dir) for laughter_info in tqdm(laughter_lines))
    else:
