...
def parallel_load_audio_batch(files, n_processes, sr=None, offsets=None, durations=None):
    if offsets is not None and durations is not None:
        return Parallel(n_jobs=n_processes)(
            delayed(librosa_load_without_sr)(files[i], sr=sr, offset=offsets[i], duration=durations[i])
            for i in tqdm(range(len(files)))
        )
    else:
        return Parallel(n_jobs=n_processes)(delayed(librosa_load_without_sr)(f, sr=sr) for f in tqdm(files))
...
def keras_pad_seqs(sequences, maxlen=None, dtype='int32', padding='pre', truncating='pre', value=0.):
    ...
    if isinstance(value, six.string_types) and dtype != object and not is_dtype_str:
        raise ValueError(
            ...
            "You should set `dtype=object` for variable length strings.".format(dtype, type(value))
        )
    ...
    for idx, s in enumerate(sequences):
        ...
        if trunc.shape[1:] != sample_shape:
            raise ValueError(
                ...
                'is different from expected shape %s' % (trunc.shape[1:], idx, sample_shape)
            )
...
def pad_sequences(sequences, pad_value=None, max_len=None):
    ...
    sequences = [librosa.util.fix_length(np.array(ft), max_len, axis=0, **kwargs) for ft in sequences]
    return sequences
...
def featurize_mfcc(f=None, offset=0, duration=None, y=None, sr=None, augment_fn=None, hop_length=None, **kwargs):
    ...
    mfcc_feat = librosa.feature.mfcc(y, sr, n_mfcc=13, n_mels=13, hop_length=hop_length, n_fft=int(sr / 40)).T  #[:,1:]
...
def featurize_audio_segments(segments, feature_fn, f=None, y=None, sr=None):
    ...
    for segment in segments:
        feature_list.append(feature_fn(f=f, offset=segment[0], duration=segment[1], y=y, sr=sr))
    return feature_list
...
def pad_sequences_with_labels(
    seq_label_tuples,
    sequence_pad_value=0,
    ...
    sequences = keras_pad_seqs(
        sequences, maxlen=max_seq_len, dtype='float32', padding='pre', truncating='post', value=sequence_pad_value
    )

    # Treat the labels the same as the input seqs if this flag is passed
    if auto_encoder_like:
        labels = keras_pad_seqs(
            labels, maxlen=max_seq_len, dtype='float32', padding='pre', truncating='post', value=sequence_pad_value
        )
...
def random_augment(y, sr, noise_signals, impulse_responses):
    ...
    functions = shuffle(
        [
            random_speed, random_stretch, random_pitch,
