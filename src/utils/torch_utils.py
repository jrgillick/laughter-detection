...
def torch_one_hot(y, device, n_dims=None):
    ...
    y_one_hot = torch.zeros(y_tensor.size()[0],
...
def run_training_loop(n_epochs,
                      model,
                      device,
                      loss_type,
                      checkpoint_dir,
...
def run_epoch(model,
    ...
    def _eval_batch(model,
        ...
        with torch.no_grad():
            ...
            output = model(src, trg,
                           0)  #[trg sent len, batch size, output dim]
            ...
            if loss_type == 'ctc':
                ...
                c = nn.CrossEntropyLoss(
                    ignore_index=0)  #output_vocab[text_utils.PAD_SYMBOL]
    ...
    if iterator is not None:
        ...
        for i, batch in tqdm(enumerate(iterator)):
            ...
            if log_frequency is not None and (model.global_step +
                                              1) % log_frequency == 0:
                ...
                if verbose:
                    ...
                    print("Train loss: ", train_loss_at_step, " | Train PPL: ",
                          train_ppl)
                    print("Val loss: ", val_loss_at_step, " | Val PPL: ",
                          val_ppl)

                writer.add_scalar('train/loss', train_loss_at_step,
                                  model.global_step)
                writer.add_scalar('eval/loss', val_loss_at_step,
                                  model.global_step)
            ...
            if checkpoint_frequency is not None and (
                    model.global_step + 1) % checkpoint_frequency == 0:
        ...
        for batch_index in range(len(batches)):
            epoch_loss += _run_batch(model, device, batches[batch_index],
                                     loss_type, teacher_forcing_ratio)
    ...
    def predict(self):
        ...
        for i_batch, batch in enumerate(self.generator):
            ...
            if labs is not None:
                ...
                labs = [
                    text_utils.readable_outputs(s, self.reverse_vocab)
                    for s in np.array(labs)
                ]

            # Run model forward
            with torch.no_grad():
                src = torch.from_numpy(np.array(seqs)).float().permute(
                    (1, 0, 2)).cpu()  #.to(device)
