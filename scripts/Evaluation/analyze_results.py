...
def get_confidence_intervals(accs, precs, recs, f1s):
    accuracy_int = ("Accuracy: %s" % ' '.join(["%f" % (x) for x in np.percentile(accs, [2.5, 50, 97.5])]) + " | Accuracy: " + str(
        np.percentile(accs, 50)) + " +- " + str((np.percentile(accs, 97.5) - np.percentile(accs, 2.5))/2))
    precision_int = ("Precision: %s" % ' '.join(["%f" % (x) for x in np.percentile(precs, [2.5, 50, 97.5])]) + " | Precision: " + str(
        np.percentile(precs, 50)) + "  +- " + str((np.percentile(precs, 97.5) - np.percentile(precs, 2.5))/2))
    recall_int = ("Recall: %s" % ' '.join(["%f" % (x) for x in np.percentile(recs, [2.5, 50, 97.5])]) + " | Recall: " + str(
        np.percentile(recs, 50)) + "  +- " + str((np.percentile(recs, 97.5) - np.percentile(recs, 2.5))/2))
    f1_int = ("F1: %s" % ' '.join(["%f" % (x) for x in np.percentile(f1s, [2.5, 50, 97.5])]) + " | F1: " + str(
        np.percentile(f1s, 50)) + "  +- " + str((np.percentile(f1s, 97.5) - np.percentile(f1s, 2.5))/2))
...
def bootstrap_metrics(tp_times, fp_times, tn_times, fn_times, n_samples=1000):
    ...
    for _ in tqdm(range(n_samples)):
        ...
        metrics = get_metrics(sample_tp_times, sample_fp_times,
                              sample_tn_times, sample_fn_times)
...
def get_event_metrics(df, cutoff_length=0.1, indices=None):
    ...
    for index in indices:
        tp, fp, tn, fn = eval_utils.get_event_metrics_per_row(
            df, index, cutoff_length=cutoff_length)
        tp_count += tp
        fp_count += fp
        tn_count += tn
        fn_count += fn
...
def bootstrap_event_metrics(df, cutoff_length=0.1, n_samples=1000):
    ...
    for _ in tqdm(range(n_samples)):
        ...
        metrics = get_event_metrics(
            df, cutoff_length=cutoff_length, indices=sample_indices)
        accuracies.append(metrics[0])
        precisions.append(metrics[1])
        recalls.append(metrics[2])
        f1s.append(metrics[3])
...
results_files_and_names.append(
    ['interannotator_agreement_results.csv', "Interannotator Agreement", 0])

results_files_and_names.append(
    ['baseline_switchboard_test_results.csv', "Baseline on SWB Test Set", 203])
results_files_and_names.append(
    ['baseline_audioset_results.csv', "Baseline on Audioset", 1000])

results_files_and_names.append(
    ['resnet_base_switchboard_test_results.csv', "Resnet Base on SWB Test Set", 203])
results_files_and_names.append(
    ['resnet_base_audioset_results.csv', "Resnet Base on Audioset", 1000])

results_files_and_names.append(
    ['resnet_specaug_wavaug_switchboard_test_results.csv', "Resnet SpecAug+WaveAug on SWB Test Set", 203])
results_files_and_names.append(
    ['resnet_specaug_wavaug_audioset_results.csv', "Resnet SpecAug+WaveAug on AudioSet", 1000])

results_files_and_names.append(['noisy_audioset_trained_resnet_specaug_wavaug_switchboard_test_results.csv',
                               "Noisy Audioset-Trained Resnet SpecAug+WaveAug on SWB Test Set", 203])
results_files_and_names.append(['noisy_audioset_trained_resnet_specaug_wavaug_audioset_results.csv',
                               "Noisy Audioset-Trained Resnet SpecAug+WaveAug on AudioSet", 1000])

results_files_and_names.append(['noisy_audioset_trained_resnet_base_switchboard_test_results.csv',
                               "Noisy Audioset-Trained Resnet Base on SWB Test Set", 203])
results_files_and_names.append(['noisy_audioset_trained_resnet_base_audioset_results.csv',
                               "Noisy Audioset-Trained Resnet Base on AudioSet", 1000])

results_files_and_names.append(['noisy_audioset_trained_baseline_switchboard_test_results.csv',
                               "Noisy Audioset-Trained Baseline on SWB Test Set", 203])
results_files_and_names.append(['noisy_audioset_trained_baseline_audioset_results.csv',
                               "Noisy Audioset-Trained Baseline on AudioSet", 1000])
...
for i in range(len(results_files_and_names)):
    ...
    if include_event_based_results:
        print("\nEvent-Based Results:")
        event_confidence_intervals = bootstrap_event_metrics(
            results_df, n_samples=n_samples, cutoff_length=0.1)
